...
export async function checkAccess({ device_id, product }) {
  ...
  if (result.rows.length === 0) {
    return { status: 'inactive', subscription_id: null, expires_at: null, payment_link_url: null };
  }

  const sub = result.rows[0];

  if (sub.status === 'trial' && sub.trial_end_date && new Date(sub.trial_end_date) > new Date()) {
    return { status: 'active', subscription_id: sub.subscription_id, expires_at: sub.trial_end_date, payment_link_url: null };
  }

  if (sub.status === 'active' && (sub.expires_at === null || new Date(sub.expires_at) > new Date())) {
    return { status: 'active', subscription_id: sub.subscription_id, expires_at: sub.expires_at, payment_link_url: null };
  }

  // Subscription is inactive/expired — return payment link if available and not expired
  const hasValidLink = sub.payment_link_url &&
    sub.payment_link_expires_at &&
    new Date(sub.payment_link_expires_at) > new Date();

  return {
    status: 'inactive',
    subscription_id: sub.subscription_id,
    expires_at: sub.expires_at,
    payment_link_url: hasValidLink ? sub.payment_link_url : null,
  };
}
